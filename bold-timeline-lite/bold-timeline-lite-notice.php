	 ...
	 class BoldTimelineLiteNotice {
			public function __construct() {				
				if( is_admin() ){					
					...
					//add_action( 'wp_ajax_bold_timeline_lite_dismiss_notice_callto_action_snooze', array( $this, 'bold_timeline_lite_dismiss_notice_callto_action_snooze_callback' ) );					
				}
			}
			...
			public function bold_timeline_lite_dismiss_notice_callto_action_snooze_callback(){
					
					set_transient ( 'bold-timeline-lite-plugin-notice-dismissed', 'dismissed', 30 * DAY_IN_SECONDS );
			...
			function bold_timeline_lite_create_notice_for_upgrade_plugin(){
					...
					$button_text_snooze	= esc_html__( 'Snooze for 30 days', 'bold-timeline' );
...
function bold_timeline_wpdocs_this_screen( $current_screen ) {    
	/*if( class_exists( 'BoldThemesFramework' )) {
		if(  $current_screen->id === "bold-timeline_page_bold-timeline-edit" || $current_screen->id === "toplevel_page_bold-timeline" ) {	
...
add_action( 'current_screen', 'bold_timeline_wpdocs_this_screen', 10, 1 );