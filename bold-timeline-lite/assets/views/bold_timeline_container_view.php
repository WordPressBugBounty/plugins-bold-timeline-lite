...
if ( ! defined( 'ABSPATH' ) ) {
	exit;
		...
		if ( $group_title_font != '' && $group_title_font != 'inherit' ) {
			...
			$el_style .= '; --group-title-font: \'' . urldecode( $group_title_font ) . '\'';
			$class[] = $prefix . '_has_group_title_font';
		}
		...
		if ( $line_color != '' ) {
			...
			$el_style .= '; --boldthemes-default-line-color: ' . $line_color;
		}
		...
		if ( $group_frame_color != '' ) {
			...
			$css_override = $css_override_temp . $css_override;*/
			$el_style .= '; --boldthemes-default-group-color: ' . $group_frame_color;
		}
