	...
	class BTBB_Light_List_Table extends WP_List_Table {
		...
		public static function define_columns() {
			$columns = array(
				...
				'title' => esc_html__( 'Title', 'bold-timeline' ),
				'shortcode' => esc_html__( 'Shortcode', 'bold-timeline' ),
				'author' => esc_html__( 'Author', 'bold-timeline' ),
				'date' => esc_html__( 'Date', 'bold-timeline' ),
			);
		...
		function get_bulk_actions() {
			$actions = array(
				'delete' => esc_html__( 'Delete', 'bold-timeline' ),
			);
			...
			if ( current_user_can( 'delete_posts', $item->id() ) ) {
				...
				$actions = array_merge( $actions, array(
					'delete' => sprintf( '<a href="%1$s" onclick="if (confirm(\'' . esc_js( esc_html__( "You are about to delete this post.\n'Cancel' to stop, 'OK' to delete.", 'bold-timeline' ) ) . '\')) {return true;} return false;">%2$s</a>',
						esc_url( $delete_link ),
						esc_html__( 'Delete', 'bold-timeline' )
					),
				) );
			}
		...
		function column_date( $item ) {
			...
			$t_time = mysql2date( esc_html__( 'Y/m/d g:i:s A', 'bold-timeline' ),
				$post->post_date, true );
