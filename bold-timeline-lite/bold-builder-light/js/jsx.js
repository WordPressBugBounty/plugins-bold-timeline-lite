...
(function( $ ) {
	...
	window.bt_bb_dialog.show = function( type ) {
		...
		// edit content
			...
			// Bold Timeline Lite has no AI content generation, so the AI Content Generator
			// UI is intentionally omitted here (bold-page-builder appends it via
			// window.bt_bb_dialog_param.ai_prompt + the parent's `ai_prompt_helper` attr).
			// Dropping it also removes the parent-node lookup that lived only to serve AI —
			// which is what previously crashed via the `$( '[data-reactid="..." ]' )` selector
			// (this React build's reactid values contain CSS-special chars `.`/`$`, so a
			// selector built from one matches nothing). The content itself is resolved from
			// state by key in the submit handler below, never by a reactid selector.
